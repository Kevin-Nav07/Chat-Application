function extractPathParams(pattern, url) {
    ...
    urlSplit = pattern.split("/");
    //

}


//register routes by passing it into a function
//the problem, we need to have a map of routes to validate an incoming route
//input: a route of format /path/path2/resource/path
//we know it is of valid format, now we want to match each route and also extract the path paramaters
//two functionalities. To match a route to an existing route, and also extract the path paramaters from that route
// we will have a seperte funciton to register routes and generate a regex pattern for a specific route
//when we have the regex for a route, we can use the regex to identify a pattern and take out the captured group value
